...
$hackathonsStmt = $pdo->prepare('SELECT id, name FROM hackathons ORDER BY starts_at DESC, id DESC');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    if (!CSRF::validate($_POST['csrf_token'] ?? null)) {
        echo json_encode(['ok' => false, 'type' => 'error', 'message' => 'Session token expired. Refresh the page and try again.']);
        exit;
    }

    if ($selectedHackathonId === null) {
        echo json_encode(['ok' => false, 'type' => 'error', 'message' => 'No hackathon selected.']);
        exit;
    }
    ...
    if ($action === 'scan') {
        ...
        $stmt->execute([$selectedHackathonId, $code, $code, normalizeVitRegNo($code)]);
        $participant = $stmt->fetch();

        if ($participant === false) {
            echo json_encode(['ok' => false, 'type' => 'error', 'message' => 'ID not found']);
            exit;
        }
    ...
    if ($action === 'manual_checkin') {
        ...
        if ($participantId === false || $participantId === null) {
            echo json_encode(['ok' => false, 'type' => 'error', 'message' => 'Invalid participant selected.']);
            exit;
        }
...
scanForm.addEventListener('submit', async (event) => {
    ...
    const response = await fetch(window.location.pathname + '?hackathon_id=' + hackathonId, { method: 'POST', body: formData });
...
setInterval(async () => {
    const response = await fetch(window.location.pathname + '?hackathon_id=' + hackathonId + '&ajax=stats');
